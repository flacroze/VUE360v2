...
export default function AgentOccupancyChart ({ filters , onDataChange, className, height } : AgentOccupancyChartProps) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded shadow-lg">
          ...
          <p className="text-sm">{`Heures planifiées: ${data.planned}h`}</p>
          <p className="text-sm">{`Heures assignées: ${data.assigned}h`}</p>
          <p className="text-sm font-semibold">{`Taux d'occupation: ${data.occupancy}%`}</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="mb-4">
        <h2 className="text-xl font-bold text-gray-800">Taux d'occupation des agents</h2>
        <p className="text-sm text-gray-600">
          Ratio heures assignées / heures planifiées par agent
        </p>
