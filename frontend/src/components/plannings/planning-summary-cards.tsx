...
import { Clock, Calendar, TrendingUp, Loader2, Activity } from "lucide-react";
import type { FilterOptions, PlanningScheduleSummary, PlanningActivitiesSummary } from "../shared/schema";
...
export default function PlanningSummaryCards({ filters }: PlanningSummaryCardsProps) {
  ...
  const { data: scheduleSummary, isLoading: isLoadingSchedule } = useQuery<PlanningScheduleSummary>({
    queryKey: ['/api/planning/schedule-summary', filters],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/planning/schedule-summary?${params}`);
  ...
  const { data: activitiesSummary, isLoading: isLoadingActivities } = useQuery<PlanningActivitiesSummary>({
    queryKey: ['/api/planning/activities-summary', filters],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (filters.siteId) params.append('siteId', filters.siteId.toString());
      if (filters.contractType) params.append('contractType', filters.contractType);
      if (filters.teamId) params.append('teamId', filters.teamId.toString());
      if (filters.groupId) params.append('groupId', filters.groupId.toString());
      if (filters.experienceId) params.append('experienceId', filters.experienceId);
      if (filters.contextId) params.append('contextId', filters.contextId);
      if (filters.startDate) params.append('startDate', filters.startDate);
      if (filters.endDate) params.append('endDate', filters.endDate);
      
      const response = await fetch(`/api/planning/activities-summary?${params}`);
      if (!response.ok) throw new Error('Failed to fetch activities summary');
      return response.json();
    },
  });
      ...
      <Card>
        ...
        <CardContent>
          {isLoadingSchedule ? (
            ...
            <>
              <div className="text-2xl font-bold">{scheduleSummary?.totalHours.toLocaleString() || 0}h</div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoadingSchedule ? (
            ...
            <>
              <div className="text-2xl font-bold">{scheduleSummary?.assignedHours?.toLocaleString() || 0}h</div>
              <p className="text-xs text-muted-foreground">
                Activités assignées
              </p>
            </>
      ...
      <Card>
        ...
        <CardContent>
          {isLoadingSchedule ? (
            ...
            <>
              <div className="text-2xl font-bold">{scheduleSummary?.utilizationRate || 0}%</div>
              <p className="text-xs text-muted-foreground">
